...
class UserCourseProgressModelTests(TestCase, TestHelperMixin):
    def setUp(self):
        ...
        self.course = Course.objects.create(name='tank', difficulty=1, is_under_construction=False,
                                            main_teacher=self.auth_user)
        self.lesson = Lesson.objects.create(lesson_number=1, course=self.course, intro='',
                                            content='', annexation='', is_under_construction=False)

    def test_normal_creation(self):
        course_progress = UserCourseProgress.objects.create(user=self.auth_user, course=self.course)

        self.assertEqual(course_progress.user, self.auth_user)
        self.assertEqual(course_progress.course, self.course)
        self.assertIn(course_progress, self.course.student_progresses.all())
